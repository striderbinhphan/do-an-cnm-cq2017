...
app.post("/register", async (req, res) => {
  ...
  await charityBlockChain.register(newUser);
...
app.post("/login", async (req, res) => {
  ...
  if (await charityBlockChain.isUserExisting(publicKey)) {
    if (ec.keyFromPrivate(privateKey, "hex").getPublic("hex") === publicKey) {
      const payload =await charityBlockChain.getUserInfo(publicKey);
      console.log(payload);
      res.json(payload[0]).end();
    } else {
...
app.get("/pending-transactions",async (req, res) => {
  return res.json(await charityBlockChain.getPendingTransactions()).end();
});
...
app.get("/projects",async (req, res) => {
  res.json(await charityBlockChain.getProjectList()).end();
});
...
app.get("/donate-projects",async (req, res) => {
...
app.get("/test",(req,res)=>{
  console.log(charityBlockChain.miningBlock());
  res.json({test:"test"});
})
//peer to peer api
